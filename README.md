<h1 align="center">Hi 👋, I'm Manvitha Ankam.</h1>

<h3 align="center">
  AI/ML Engineer · Data Engineering Enthusiast
</h3>

<p align="center">
  Building intelligent systems and data-driven solutions 🚀
</p>

<p align="center">
  <a href="YOUR_LINKEDIN_URL">LinkedIn</a> •
  <a href="https://github.com/Manvitha975">GitHub</a>
</p>

---

### 🧠 Interests

`Machine Learning` • `Generative AI` • `LLMs` • `AI Agents`  
`Data Engineering` • `Data Analytics` • `MLOps`

### 🛠️ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,fastapi,mysql,postgres,gcp,aws,docker,git,github" />
</p>

<p align="center">
  <sub>Python • SQL • TensorFlow • PyTorch • FastAPI • BigQuery • Docker • Git</sub>
</p>
